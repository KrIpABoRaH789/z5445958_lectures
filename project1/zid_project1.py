...
def read_dat(tic):
    ...
    with open(pathToTic, "r") as filehandle:
        for line in filehandle:
            line = line.rstrip()
            result.append(line)
    return result
...
def verify_tickers(tic_exchange_dic, tickers_lst=None):
    ...
    if tickers_lst is None:
        ...
        # Check if tickers_lst is an empty list.
    if not tickers_lst:
        raise Exception("tickers_lst is an empty list.")

        # Check if tickers_lst contains a ticker that does not correspond to a key in tic_exchange_dic.
    for ticker in tickers_lst:
        if ticker not in tic_exchange_dic:
            raise Exception(f"Ticker {tic} is not valid.")
...
def verify_cols(col_lst=None):
    ...
    if col_lst is None:
        ...
        # Check if col_lst is an empty list.
    if not col_lst:
        raise Exception("col_lst is an empty list.")

        # Check if col_lst contains a column that is not found in `COLUMNS`.
    for col in col_lst:
        if col not in COLUMNS:
            raise Exception(f"Column {col} is not valid.")
...
def create_data_dict(tic_exchange_dic, tickers_lst=None, col_lst=None):
    ...
    """
    # Check if tickers_lst contains any invalid tickers.
    verify_tickers(tic_exchange_dic, tickers_lst)

    # Check if col_lst contains any invalid column names.
    verify_cols(col_lst)

    # If tickers_lst is None, set tickers_lst to the list of all tickers in tic_exchange_dic.
    if tickers_lst is None:
        tickers_lst = list(tic_exchange_dic.keys())

    # If col_lst is None, set col_lst to COLUMNS.
    if col_lst is None:
        col_lst = COLUMNS

    # Create a dictionary to store the data for the tickers specified in tickers_lst.
    data_dict = {}

    # Iterate over the tickers in tickers_lst.
    for ticker in tickers_lst:

        # Read the data for the ticker from the data file.
        data = read_dat(ticker)

        # Convert the data to a list of dictionaries, only keeping the columns specified in col_lst.
        data_list = []
        for line in data:
            line_dict = line_to_dict(line)
            data_list.append({key: value for key, value in line_dict.items() if key in col_lst})

        # Get the exchange for the ticker.
        tic_exchange = tic_exchange_dic[ticker]

        # Add the data for the ticker to the data dictionary.
        data_dict[ticker] = {
            "exchange": tic_exchange,
            "data": data_list
        }
